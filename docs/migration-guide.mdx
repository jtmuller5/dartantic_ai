...
- Provider lookup by name or by using intrinsic providers
  ...
  - ChatProviders.byName(name|alias), e.g. ChatProviders.byName('anthropic') or ChatProviders.byName('claude')
  - EmbeddingsProvider.name, e.g. ChatProviders.openai
  - EmebeddingsProviders.byName(name|alias), e.g. EmbeddingsProvider.forName['openai']
...
-The `Agent.providerFor()` static method has been removed.
    final provider = ChatProviders.byName('openai');
